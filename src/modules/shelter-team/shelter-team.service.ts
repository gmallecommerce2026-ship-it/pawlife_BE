...
export class ShelterTeamService {
    ...
    async inviteMember(shelterId: string, inviterId: string, dto: { email: string; role: ShelterStaffRole; name?: string }) {
        ...
        try {
            await this.mailerService.sendMail({ to: dto.email, subject, html });
        } catch (err) {
            // Log chi tiết + xoá invitation vừa tạo để tránh "kẹt" trạng thái PENDING mà user không nhận được mail
            console.error('Gửi email mời thất bại:', err);
            await this.prisma.shelterInvitation.delete({ where: { id: invitation.id } });
            throw new BadRequestException(
                'Không thể gửi email mời. Vui lòng kiểm tra lại cấu hình email hoặc thử lại sau.',
            );
        }
