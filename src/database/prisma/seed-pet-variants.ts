...
async function main() {
  ...
  // Dùng Promise.all với chunking (chia nhỏ mảng) để xử lý hệ thống lớn (hàng chục nghìn record)
  const chunkSize = 100;
  for (let i = 0; i < pets.length; i += chunkSize) {
    const chunk = pets.slice(i, i + chunkSize);
    
    await Promise.all(chunk.map(async (pet) => {
      // 1. Random Good With (1-3 items)
      const goodWith = getRandomItems(GOOD_WITH_OPTIONS, 1, 3);
      
      // 2. Random Bad With (0-2 items, lọc bỏ những cái đã có trong goodWith)
      ...
      await prisma.pet.update({
        where: { id: pet.id },
        data: {
          goodWith: goodWith, // Prisma tự parse mảng string thành JSON
          badWith: badWith,
          idealHome: idealHome,
        }
      });
